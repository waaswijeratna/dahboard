...
let userData: { id: string; name: string; email: string; age: number; pfpUrl: string; role: string;  } | null = null;

// Function to save user data to local storage
const saveUserDataToLocalStorage = (user: { id: string; name: string; email: string; age: number; pfpUrl: string; role: string; }) => {
...
const decodeToken = (token: string) => {
  if (!token || typeof token !== 'string' || !token.includes('.')) {
    console.error('Invalid token format: Token must be a string with at least one dot');
    return null;
  }
  ...
  try {
    ...
    if (parts.length !== 3) {
      console.error('Invalid token format: Token must have three parts (header.payload.signature)');
      return null;
    }
    ...
    for (const field of requiredFields) {
      if (!payload[field]) {
        console.error(`Invalid token payload: Missing required field "${field}"`);
        return null;
      }
    }
